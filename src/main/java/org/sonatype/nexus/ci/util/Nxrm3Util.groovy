...
class Nxrm3Util
{
  ...
  static List<Repository> getApplicableRepositories(final String nexusInstanceId) {
    ...
    if (configuration.version != NEXUS_3) {
      throw new IllegalArgumentException('Specified nexus instance is not a 3.x server')
    }
  ...
  {
    // TODO kris: currently nxrm does not return the repo policy (snapshot v release), how do we want to handle it?
